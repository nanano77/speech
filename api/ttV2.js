...
${lastCandidates.map((id, i) => `${i + 1}. ${layerMap[id]} => ${id}`).join("\n")}`
  ...
  ? `你是一個圖層控制助理。使用者剛剛收到候選圖層清單如下，請根據使用者輸入選擇對應圖層，並回傳以下 JSON 格式：

{
  "intent": "open" 或 "close",
  "targets": ["圖層代碼"] // ✅ 必須是圖層代碼（如下方候選）
}

⚠️ 僅允許使用候選清單中的圖層代碼為 targets，禁止使用 null、文字名稱、空值或非候選圖層。

候選清單如下：
${lastCandidates.map((id, i) => `${i + 1}. ${layerMap[id]} => ${id}`).join("\n")}

使用者語句可能會是：「第一個」、「我要看第 2 和第 3 個」、「全部」、「除了第二個」等。
請從上方候選圖層中選擇對應的圖層代碼作為 targets。`
